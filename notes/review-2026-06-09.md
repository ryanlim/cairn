...
**Remaining** (deferred — larger UI features that need on-device iteration):
- **5.1 / H1** — server-fetch progress invisible outside InitialScanScreen
  (the "frozen Processing N/N" during the post-hash server fetch). Needs
  `.fetchingServer` driven as a real phase + `serverAssetsFetched` surfaced
  into StatusScreen's sync card + SyncDetailSheet, and a per-batch callback
  on ServerAssetSyncCoordinator. Touches view layout — verify live.
- **5.4 / M1+M2** — initial-scan ETA rebaselines from `initialHashed: 0`
  after imputation (poisons the persisted per-asset rate) and the bar
  fills to 100% twice with a backward snap (imputation reuses
  syncProgress.hashed as "scanned"). Needs a separate `scanned` field /
  phase-scoped progress; verify the counter live.
- Minor: M8 (export/import spinner), L2 (suppressErrors alert
  resurrection), L3/L4/L5 (busy states / stale toasts).
