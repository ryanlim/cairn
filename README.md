...
`cairn --help` enumerates every subcommand and flag. API key scopes needed: `asset.read`, `asset.delete`, `tag.create`, `tag.asset` — plus `tag.read` for `history` and `restore --file-name-matches`.
