...
1. **Live Photo delete cascade.** Does `DELETE /api/assets` with a still-image UUID also trash the linked motion video via `livePhotoVideoId`, or must both UUIDs be in the request? If the former, our diff can key on the still's checksum alone. If the latter, we need to resolve `livePhotoVideoId` and include it in every delete batch.
